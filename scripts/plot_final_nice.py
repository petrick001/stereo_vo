...
def prepare_room2(gt_path, mono_path, stereo_path):
    ...
    stereo_xyz = np.array([stereo[j, 1:4] for _, j in pairs_s])

    s_m, R_m, t_m = sim3_align(gt_m, mono_xyz)
    mono_aligned = (s_m * (R_m @ mono_xyz.T)).T + t_m

    R_s, t_s = rigid_align_3d(gt_s, stereo_xyz)
    stereo_aligned = (R_s @ stereo_xyz.T).T + t_s

    rmse_m = np.sqrt(np.mean(np.sum((gt_m - mono_aligned) ** 2, axis=1)))
    rmse_s = np.sqrt(np.mean(np.sum((gt_s - stereo_aligned) ** 2, axis=1)))
...
def plot_room2_mono(data, out_path):
    ...
    plt.plot(data["gx"], data["gz"], label="Ground Truth", linewidth=3)
    plt.plot(
        data["mx"],
        data["mz"],
...
def plot_combined_start_aligned(data, label_a, label_b, title, out_path):
    ...
    ax.plot(
        data["ax"],
        data["az"],
        label=f"{label_a} (drift={data['drift_a']:.2f} m)",
        linewidth=2.8,
    )
    ax.plot(
        data["bx"],
        data["bz"],
        label=f"{label_b} (drift={data['drift_b']:.2f} m)",
        linewidth=2.8,
    )
...
def main():
    ...
    plot_single_start_aligned(
        ...
        "Monocular VO",
        corr["drift_a"],
        "Corridor3 - Monocular VO",
        out_dir / "corridor3_mono.png",
    )
    plot_single_start_aligned(
        corr["bx"],
        corr["bz"],
    ...
    plot_combined_start_aligned(
        ...
        "Stereo VO",
        "Corridor3 - Combined",
        out_dir / "corridor3_combined.png",
    )

    outdoors = prepare_start_aligned(args.outdoors5_mono, args.outdoors5_stereo)
    plot_single_start_aligned(
        outdoors["ax"],
        outdoors["az"],
        "Monocular VO",
        outdoors["drift_a"],
        "Outdoors5 - Monocular VO",
        out_dir / "outdoors5_mono.png",
    )
    plot_single_start_aligned(
        outdoors["bx"],
        outdoors["bz"],
    ...
    plot_combined_start_aligned(
        ...
        "Stereo VO",
        "Outdoors5 - Combined",
        out_dir / "outdoors5_combined.png",
    )
